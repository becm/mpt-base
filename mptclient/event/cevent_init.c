 ...
 */
extern int mpt_cevent_init(MPT_INTERFACE(client) *solv, MPT_STRUCT(event) *ev)
{
	...
	if (ev->msg) {
		...
		while (part > 0) {
			if (!(part = mpt_message_pset(&msg, mt.arg, setConfig, &solv->conf))) {
				mpt_output_log(solv->out, __func__, MPT_ENUM(LogError), "%s", MPT_tr("error while setting client configuration"));
				part = 1;
			}
		}
	}
	...
	mpt_output_log(solv->out, __func__, 0, 0);
