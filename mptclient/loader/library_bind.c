...
extern int mpt_library_bind(MPT_STRUCT(proxy) *px, const char *conf, MPT_INTERFACE(logger) *out)
{
	...
	if (!conf) {
		(void) mpt_log(out, __func__, MPT_ENUM(LogError), "%s", MPT_tr("missing initializer target"));
		return -1;
	}
	...
	if (!*ldesc) {
		...
		(void) mpt_log(out, __func__, MPT_ENUM(LogError), "%s: %s", MPT_tr("invalid initializer length"), conf);
		return 0;
	}
	else {
	...
	if ((m = px->_mt) && (id == px->_id)) {
		(void) mpt_log(out, __func__, MPT_ENUM(LogDebug), "%s: %s", MPT_tr("instance types match"), conf);
		return 0;
	}
	/* create new proxy */
