...
{
    Slice<linepart> d = data();
    return mpt_linepart_ulength(d.base(), d.len());
}
size_t Polyline::Parts::rawLength()
{
    Slice<linepart> d = data();
    return mpt_linepart_rlength(d.base(), d.len());
}


Polyline::Polyline(int dim) : _rawSize(sizeof(double)), _dataSize(2*sizeof(double))
{
...
ssize_t Polyline::truncate(int dim, ssize_t pos)
{
    ...
    if (pos < 0) return raw->used() / _rawSize;
...
Slice<dpoint> Polyline::values(int part) const
{
    const array *arr;
    size_t len;
    Slice<linepart> pt = vis();

    if (!(arr = userData()) || !(len = pt.len()) || part >= (int) len) {
        return Slice<dpoint>(0, 0);
    }

    dpoint *base = (dpoint *) arr->base();
    size_t used = arr->used() / sizeof(dpoint);
...
Slice<linepart> Polyline::vis() const
{
    array *a;
    if (!(a = _d.get(0))) {
        return Slice<linepart>(0, 0);
    }
...
int applyLineData(dpoint *dest, const linepart *lp, int plen, const Transform &tr, polyline &part)
{
    ...
    for (int i = 0; i < dim; i++) {
        ...
        if (lp) {
            ...
            while (plen > std::numeric_limits<typeof(tmp.usr)>::max()) {
                tmp.usr = tmp.raw = std::numeric_limits<typeof(tmp.usr)>::max();
                plen -= tmp.usr;
...
void Polyline::transform(const Transform &tr)
{
    ...
    Slice<linepart> lp = a->data();
    applyLineData(pts, lp.base(), lp.len(), tr, *this);

    _modified = 0;
}

Slice<dpoint> Polyline::Points::data() const
{
    return Slice<dpoint>((dpoint *) base(), used() / sizeof(dpoint));
}
